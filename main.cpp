...
// Modulo de Mouse
char mouse_reset_and_status(unsigned short* num_buttons); // Devuelve char (0 o 1)
...
DiskStatus disco_reset_disk_system(unsigned char param_drive) {
    ...
    DiskStatus status_ret; // Renombrar para evitar conflicto
    unsigned char ah_res;  // Renombrar
...
// =============================================
PrinterStatus impresora_initialize(unsigned char param_printer_num) {
    cout << "IMPRESORA: Llamando INT 17h AH=01h - Inicializar LPT" << (int)param_printer_num + 1 << "." << endl;
    PrinterStatus status_ret;
    unsigned char ah_res;
    unsigned short dx_param = param_printer_num; // Pasar como short a ASM
...
PrinterStatus impresora_get_status(unsigned char param_printer_num) {
    cout << "IMPRESORA: Llamando INT 17h AH=02h - Obtener Estado LPT" << (int)param_printer_num + 1 << "." << endl;
    PrinterStatus status_ret;
    unsigned char ah_res;
    unsigned short dx_param = param_printer_num;
    ...
    // ... (mas interpretacion de bits si se desea)
    print_status_bios("INT 17h AH=02h ejecutada.", status_ret.success_flag, 0);
    return status_ret;
}

PrinterStatus impresora_send_char(unsigned char param_char, unsigned char param_printer_num) {
    cout << "IMPRESORA: Llamando INT 17h AH=00h - Enviar Char '" << param_char << "' a LPT" << (int)param_printer_num + 1 << "." << endl;
    PrinterStatus status_ret;
    unsigned char ah_res;
    unsigned short dx_param = param_printer_num;
    print_regs_bios("INT 17h AH=00h (Entrada)", (0x0000 | param_char), 0, 0, dx_param);
    asm {
        mov ah, 0x00
        mov al, param_char
        mov dx, dx_param
    ...
    status_ret.status_byte = ah_res;
    status_ret.success_flag = !(ah_res & 0x08) ? 1 : 0;
    print_regs_bios("INT 17h AH=00h (Salida)", ((unsigned short)ah_res << 8) | param_char, 0, 0, dx_param);
...
void menu_pantalla_dos() {
    ...
    char char_t=' ';
    unsigned int temp_input_val; // Renombrar para evitar conflictos
    char dummy_buf[80];
    ...
    do {
        ...
        switch (opcion_menu) {
            case 1:
                cout << "Modo de video (hex): "; cin >> hex >> temp_input_val; mode_t = (unsigned char)temp_input_val;
                while(cin.get()!='\n');
                pantalla_set_video_mode(mode_t);
                break;
            case 2:
                cout << "Linea inicio cursor (scanline): "; cin >> dec >> temp_input_val; r_t = (unsigned char)temp_input_val;
                cout << "Linea fin cursor (scanline): "; cin >> temp_input_val; c_t = (unsigned char)temp_input_val;
                while(cin.get()!='\n');
                pantalla_set_cursor_type(r_t, c_t);
                break;
            case 3:
                cout << "Fila: "; cin >> dec >> temp_input_val; r_t = (unsigned char)temp_input_val;
                cout << "Columna: "; cin >> temp_input_val; c_t = (unsigned char)temp_input_val;
                while(cin.get()!='\n');
            ...
            case 4:
                ...
                break;
            case 5:
                cout << "Caracter: "; cin >> char_t;
                cout << "Atributo (hex): "; cin >> hex >> temp_input_val; attr_t = (unsigned char)temp_input_val;
                cout << "Veces (dec): "; cin >> dec >> times_t;
                while(cin.get()!='\n');
                pantalla_write_char_attr(char_t, attr_t, 0, times_t);
                break;
            case 6: pantalla_clear_screen(); break;
            case 7:
                cout << "Lineas a scrollear (ej.1): "; cin >> dec >> temp_input_val; r_t = (unsigned char)temp_input_val;
                cout << "Atributo para lineas nuevas (hex, ej.07): "; cin >> hex >> temp_input_val; attr_t = (unsigned char)temp_input_val;
                while(cin.get()!='\n');
...
void menu_disco_dos() {
    int opcion_menu;
    unsigned char drive_param = 0x80;
    ...
    do {
        ...
        switch(opcion_menu) {
            case 1:
                ...
                cin >> hex >> temp_in; drive_param = (unsigned char)temp_in;
                while(cin.get()!='\n');
...
void menu_raton_dos() {
    int opcion_menu;
    unsigned short num_botones_m=0, x_m_val=0, y_m_val=0, stat_m_val=0; // Renombrar
    char dummy_b_m[80];
    ...
    do {
        ...
        switch(opcion_menu) {
            case 1: mouse_show_cursor(); break;
            case 2: mouse_hide_cursor(); break;
            case 3:
                mouse_get_position_and_status(&x_m_val, &y_m_val, &stat_m_val);
                cout << "  En C++: X=" << x_m_val << " Y=" << y_m_val << " Botones=0x" << hex << stat_m_val << dec << endl;
                break;
            case 4:
                cout << "Nueva X (0-639): "; cin >> x_m_val;
                cout << "Nueva Y (0-199 o 0-349 para texto): "; cin >> y_m_val;
                while(cin.get()!='\n');
...
void menu_impresora_dos() {
    int opcion_menu;
    unsigned char prn_p_val = 0; // Renombrar
    char char_p_val;             // Renombrar
    unsigned int temp_in_p;      // Renombrar
    char dummy_b_p[80];
    ...
    do {
        ...
        switch(opcion_menu) {
            case 1:
                cout << "Numero impresora (0=LPT1, 1=LPT2, 2=LPT3): "; cin >> temp_in_p;
                if (temp_in_p <=2) prn_p_val = (unsigned char)temp_in_p; else cout << "Num invalido\n";
                while(cin.get()!='\n');
                impresora_initialize(prn_p_val);
                break;
            case 2:
                cout << "Numero impresora (0-2): "; cin >> temp_in_p;
                if (temp_in_p <=2) prn_p_val = (unsigned char)temp_in_p; else cout << "Num invalido\n";
                while(cin.get()!='\n');
            ...
            case 3:
                cout << "Numero impresora (0-2): "; cin >> temp_in_p;
                if (temp_in_p <=2) prn_p_val = (unsigned char)temp_in_p; else cout << "Num invalido\n";
                cout << "Caracter a enviar: "; cin >> char_p_val;
                while(cin.get()!='\n');
